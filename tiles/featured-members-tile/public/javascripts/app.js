...
/***/ function(module, exports, __webpack_require__) {
	...
	var Title = React.createClass({
	    ...
	    render: function () {
	        return React.createElement(
	            'div',
	            { className: 'item-container bg-color-gray' },
	            React.createElement(
	                'h2',
	                { className: 'title' },
	                'Title'
	            ),
	            React.createElement('input', { type: 'text', placeholder: 'Max 50 Characters', value: this.props.state.tileTitle, onChange: this.onChangeTitle })
	        );
	    }
	});
	...
	var Header = React.createClass({
	    ...
	    render: function () {
	        return React.createElement(
	            'div',
	            { className: 'row margin-bottom-sm  margin-top-lg' },
	            React.createElement(
	                'div',
	                { className: 'col-6' },
	                React.createElement(
	...
	var MembersValidation = React.createClass({
	    displayName: 'MembersValidation',

	    render: function () {
	        return React.createElement(
	            'div',
	            { className: 'inline-error-message atleast-one-member' },
	...
	var RequiredValidation = React.createClass({
	    displayName: 'RequiredValidation',
	...
	var ValidUserValidation = React.createClass({
	    displayName: 'ValidUserValidation',
	...
	var alreadyExistValidation = React.createClass({
	    displayName: 'alreadyExistValidation',

	    render: function () {
	        return React.createElement(
	            'div',
	            { className: 'inline-error-message' },
	            'Selected member already exist'
	        );
	    }
	});

	var MemberConfig = React.createClass({
	    displayName: 'MemberConfig',
	    ...
	    getTitle: function (profile) {
	        return _.pluck(_.filter(profile, function (e) {
	            return e.jive_label == "Title";
	        }), 'value')[0] || '';
	    },
	    ...
	    selectMember: function (e) {
	        ...
	        $(e.target).autocomplete({
	            ...
	            return $("<li>").data("ui-autocomplete-item", item).append("<div><img class='avatar-img' src='" + item.avatarUrl + "'>&nbsp;<span class='name-label'>" + item.label + "</span></div>").appendTo(ul);
	        };
	    },
	    ...
	    checkEmptyValidation: function () {
	        return this.props.member.displayName.length == 0 && this.props.state.validation ? React.createElement(RequiredValidation, null) : null;
	    },

	    validMemberValidation: function () {
	        return !this.props.member.validUser && this.props.state.validation ? React.createElement(ValidUserValidation, null) : null;
	    },
	    ...
	    render: function () {
	        ...
	        return React.createElement(
	            'div',
	            { className: 'row margin-bottom-sm', 'data-docid': this.props.member.id },
	            React.createElement('i', { className: 'fa fa-ellipsis-v dragable-icon', ref: 'ItemHandle' }),
	            React.createElement(
	                'div',
	                null,
	                React.createElement(
	                    'div',
	                    { className: 'col-6' },
	                    React.createElement('input', { type: 'text', value: this.props.member.displayName, style: inputStyle, onChange: this.selectMember }),
	                    React.createElement(
	                        'div',
	                        { className: 'inline-error-message', id: 'member-exist-error', style: { display: 'none' } },
	                        'Selected member already exist.'
	                    ),
	                    validations,
	                    React.createElement(
	                        'div',
	                        { className: 'profile-data', style: blockStyle },
	                        React.createElement('img', { className: 'avatar', src: this.props.member.avatarUrl }),
	                        React.createElement(
	                            'label',
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        ...
	                        React.createElement('input', { type: 'text', placeholder: 'Max 50 characters', value: this.props.member.title, onChange: this.changeTitle })
	                    ),
	                    React.createElement('i', { className: 'fa fa-trash fa-lg trash-icon', onClick: this.deleteMember })
	                )
	            )
	        );
	    }
	...
	var AddMember = React.createClass({
	    ...
	    getNextId: function () {
	        if (this.props.state.users.length == 0) return 1;else {
	            var maxId = _.reduce(this.props.state.users, function (max, n) {
	                if (max.id > n.id) return max;else return n;
	            });
	        }
	        return maxId.id + 1;
	    },
	    ...
	    render: function () {
	        ...
	        return React.createElement(
	            ...
	            React.createElement(
	                'a',
	                { className: this.props.state.users.length < 15 ? "add-button" : "add-button disabled", href: '#', onClick: this.addMember },
	                '+ Add'
	            ),
	            React.createElement(
	                'span',
	...
	var SaveConfig = React.createClass({
	    ...
	    render: function () {
	        return React.createElement(
	            ...
	            React.createElement(
	                'a',
	                { href: '#', className: 'btn btn-primary', onClick: this.applyChanges },
	                'Apply'
	            ),
	            React.createElement(
	                'a',
	                { href: '#', className: 'btn btn-secondary margin-left-md', onClick: this.cancelChanges },
	                'Cancel'
	            )
	        );
	    }
	...
	var MainConfig = React.createClass({
	    ...
	    render: function () {
	        var users = [];
	        var memberValidation = this.state.users.length == 0 && this.state.validation ? React.createElement(MembersValidation, null) : null;
	        if (this.state.users) {
	            users = this.state.users.map(function (member, idx) {
	                return React.createElement(MemberConfig, _extends({ updateState: this.updateMainState,
	        ...
	        return React.createElement(
	            ...
	            memberValidation,
	            React.createElement(Title, { updateState: this.updateMainState, state: this.state }),
	            React.createElement(
	                'div',
	                { className: 'item-container bg-color-gray' },
	                ...
	                React.createElement(AddMember, { updateState: this.updateMainState, state: this.state })
	            ),
	            React.createElement(SaveConfig, { updateState: this.updateMainState, state: this.state })
	        );
	    }
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var listMixin = {
	  _hasOption: function (name) {
	    return this.listOptions && typeof this.listOptions[name] != "undefined";
	  },
	  getInitialState: function () {
	  ...
	  bindMove: function (movedComponent, moveElemEvent) {
	    ...
	    this.moveHandler = function (e) {
	      ...
	      for (i = 0, len = siblings.length; i < len; i++) {
	        sibling = siblings[i];
	        if (sibling !== this.intersectItem && sibling !== moveElem) {
	...
	var itemMixin = {
	  ...
	  insertPlaceHolder: function (el) {
	    ...
	    parentEl.insertBefore(this.placeholder, newIndex > elIndex ? el : el.nextSibling);
	  },
